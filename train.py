...
# [state, p.getActionSet().index(action), reward, next_state, done]
...
def train(replay_memory, model, target_model):
    ...
    #output shape: 10 (batch size) by 5 (action shape)
    x = []
    y = []
...
for episode in range(train_episodes):
    ...
    for frame in range(frames_per_episode):
        ...
        if rand_nb <= epsilon:  # explore
            action = p.getActionSet()[np.random.randint(0, action_shape)] #check if agent chooses last action (stays in place)
        ...
        replay_memory.append([state, p.getActionSet().index(action), reward, next_state, done]) #check if it is indexOf or index
